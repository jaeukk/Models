...
void RSA::RSA_II_Serial(size_t NumInsertedLimit, size_t TrialTime)
{
	...
	while(nbrinserted>NumInsertedLimit)
	{
		...
		for(size_t i=0; i<TrialTime; i++)
		{
			//for(int j=0; j<this->dimension; j++) //why??
...
//Insert spheres of a specified radius for the trial time
//when the inserted particle for given trials is smaller than [NumInsertedLimit]
//voxels are splitted
...
SpherePacking GenerateRSAPacking(int dimension, unsigned long nbrsphere, short nbrinsertedlimit, unsigned long trial1, unsigned long trial2, double voxelratio, int seed, std::ostream & output, double * volumeratio, bool Verbose, size_t NumCut)
{
	...
	con1.gen.seed(seed);
	//add particles until accepted trials is less than [nbrinsertedlimit]
	...
	int SplitCount=0;
	int Factor = pow(2, dimension); // Multiplies voxel lists
	while(con1.pVoxels->NumVoxel()!=0)
	{
		...
		//To prevent memory space from being depleted
		if (sizeof(VoxelList) * temp * Factor < MAX_MEM_BOX) {
			con1.SplitVoxel(parallel, 1);
			SplitCount++;
			if (Verbose > 3)
				output << "voxel list is splitted \n";
		}
		else
			if (Verbose > 3)
				output << "currently there are too many voxel lists (" << temp << ")\n";
...
double Rc(DimensionType d, double numDensity) {
	...
	r = pow(volumeFraction / (numDensity * HyperSphere_Volume((int)d, 1.0)), 1.0 / (double)d);
	return r;
}